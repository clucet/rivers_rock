...
from logoutils import pillow_grain_overlay, BEBAS_PATH, MONTSERRAT_PATH, ORBITRON_PATH, RAJDHANI_PATH, JETBRAINS_PATH
...
def gen_social():
    ...
    bbox = draw.textbbox((0, 0), "PROCHAIN CONCERT", font=font_raj)
    tw = bbox[2] - bbox[0]
    draw.text(((w - tw) / 2, 380), "PROCHAIN CONCERT", fill=CYAN_PIL, font=font_raj)
    bbox = draw.textbbox((0, 0), "[DATE]", font=font_orb_m)
    tw = bbox[2] - bbox[0]
    draw.text(((w - tw) / 2, 440), "[DATE]", fill=BLANC_PIL, font=font_orb_m)
    bbox = draw.textbbox((0, 0), "[LIEU]", font=font_raj)
    tw = bbox[2] - bbox[0]
    draw.text(((w - tw) / 2, 520), "[LIEU]", fill=(180, 180, 200), font=font_raj)

    qx, qy, qs = w / 2 - 60, 660, 120
    draw.rectangle([qx, qy, qx + qs, qy + qs], fill=BLANC_PIL, outline=CYAN_PIL, width=3)
    bbox = draw.textbbox((0, 0), "QR", font=font_raj)
    tw = bbox[2] - bbox[0]
    th = bbox[3] - bbox[1]
    ...
    bbox = draw.textbbox((0, 0), "[DATE]", font=font_orb_d)
    tw = bbox[2] - bbox[0]
    draw.text(((w - tw) / 2, 650), "[DATE]", fill=ROSE_PIL, font=font_orb_d)
    bbox = draw.textbbox((0, 0), "[LIEU]", font=font_raj)
    tw = bbox[2] - bbox[0]
    draw.text(((w - tw) / 2, 840), "[LIEU]", fill=BLANC_PIL, font=font_raj)
...
.circle{fill:none;stroke:#00F5FF;stroke-width:3;stroke-dasharray:200;stroke-dashoffset:200;animation:drawC .8s ease-out .3s forwards;filter:drop-shadow(0 0 12px rgba(0,245,255,0.6))}
.lightning{fill:none;stroke:#FF2D95;stroke-width:3;stroke-linecap:round;opacity:0;animation:fadeL .5s ease-out 1.5s forwards;filter:drop-shadow(0 0 8px rgba(255,45,149,0.8))}
.center{opacity:0;animation:fadeC .4s ease-out 1.3s forwards}
@keyframes drawC{to{stroke-dashoffset:0}}
@keyframes fadeL{to{opacity:1}}
...
<svg viewBox="-200 -200 400 400">
  ...
  <polyline class="lightning" points="35,-20 60,-10 45,5 55,10"/>
  <polyline class="lightning" points="35,20 55,10 45,15 60,30"/>
  <polyline class="lightning" points="-35,-20 -50,-10 -40,5 -55,10"/>
  <polyline class="lightning" points="-35,20 -55,10 -45,15 -60,30"/>
  <circle class="center" cx="0" cy="0" r="10" fill="#FF2D95"/>
...
g.addColorStop(0,'rgba(255,45,149,0.1)');g.addColorStop(0.5,'rgba(0,245,255,0.04)');g.addColorStop(1,'rgba(15,11,26,0)');
...
.hero .tagline{font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--rose);letter-spacing:3px;text-transform:uppercase;margin-bottom:28px;text-shadow:0 0 10px rgba(255,45,149,0.3)}
...
.section h2{font-family:'Orbitron',sans-serif;font-size:26px;letter-spacing:2px;text-transform:uppercase;color:var(--rose);margin-bottom:32px;text-align:center}
...
.contact-info .email{font-family:'JetBrains Mono',monospace;font-size:14px;color:var(--cyan);text-decoration:none;transition:.2s}
