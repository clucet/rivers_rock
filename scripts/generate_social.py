...
def generate_post():
    ...
    bbox = draw.textbbox((0, 0), "[DATE]", font=font_anton_date)
    tw = bbox[2] - bbox[0]
    draw.text(((w - tw) / 2, 470), "[DATE]", fill=OR, font=font_anton_date)

    bbox = draw.textbbox((0, 0), "[LIEU]", font=font_mont)
    tw = bbox[2] - bbox[0]
    draw.text(((w - tw) / 2, 560), "[LIEU]", fill=BLANC, font=font_mont)
